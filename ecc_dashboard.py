        ...
        for item in sorted(os.listdir(agents_dir)):
            ...
            if content.startswith('---'):
                ...
                if end != -1:
                    ...
                    # Pre-compiled regex for speed, matching frontmatter keys
                    if not hasattr(load_agents, '_name_re'):
                        setattr(load_agents, '_name_re', re.compile(r'^(?:\s*)name:\s*(.*)', re.MULTILINE))
                        setattr(load_agents, '_desc_re', re.compile(r'^(?:\s*)description:\s*(.*)', re.MULTILINE))

                    name_match = getattr(load_agents, '_name_re').search(fm_content)
                    if name_match:
                        name = name_match.group(1).strip().strip('"\'')

                    desc_match = getattr(load_agents, '_desc_re').search(fm_content)
                    if desc_match:
